...
import {
  ...
  Scatter, ScatterChart, ReferenceLine, Label, LineChart, Line
} from "recharts";
...
function getWordPairData(answers: number[]) {
  // Agrupa por code (PQ, HQI, HQS, ATT)
  const data = ATTRAKDIFF_QUESTIONS.map((q, idx) => ({
    label: q.label,
    code: q.code,
    value: answers[idx] - 4, // center to 0
    display: q.label.replace("—", "-"),
  }));
  return data;
}

// Para o gráfico de médias PQ, HQ, ATT (HQ = média HQI e HQS)
...
// Para a matriz de portfólio AttrakDiff (HQ=hedonic, PQ=pragmatic)
function getPortfolioQuadrant(hq: number, pq: number) {
  // Verifica o quadrante para o label
...
export default function AttrakDiffChart({ answers }: Props) {
  ...
  return (
    <div className="w-full flex flex-col gap-8 items-center">
      {/* Gráfico tipo word-pair */}
      <Card className="w-full max-w-3xl p-6 shadow-lg">
        <h3 className="font-bold text-xl mb-2">Descrição dos Pares de Palavras</h3>
        <div className="mb-2 text-gray-500 text-sm">Pontuação média centralizada (negativo = avaliação ruim, positivo = avaliação boa)</div>
      ...
      <Card className="w-full max-w-lg p-6 shadow-lg">
        ...
        <ResponsiveContainer width="100%" height={340}>
          <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
            {/* Quadrantes (labels e fundos) */}
            {/* Gera fundo cinza claro para cada célula */}
            {[0,1,2].map(row => 
              [0,1,2].map(col => {
                // Posição (x: PQ, y: HQ), ambos vão de -2 a 2, step 2
                const x1 = -3 + col * 3;
                const x2 = x1 + 3;
                const y1 = -3 + row * 3;
                const y2 = y1 + 3;
                // Label para cada célula baseado nas imagens
                let cellLabel = "";
